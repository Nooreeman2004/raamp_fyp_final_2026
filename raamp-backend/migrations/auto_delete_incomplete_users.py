"""
Auto-delete users with incomplete onboarding (no business data)
WARNING: This script runs WITHOUT confirmation and permanently deletes users.
"""
import asyncio
...
async def auto_delete_incomplete_users():
    """Automatically delete users who have no business data (incomplete onboarding)"""
    ...
    print(f"\n🗑️  DELETING {len(users_to_delete)} INCOMPLETE USERS...")
    print("=" * 80)
    ...
    print(f"   - Remaining users: {len(users_to_keep)} users")
    
    if users_to_keep:
        print(f"\n✅ KEPT USERS WITH COMPLETE DATA:")
...
if __name__ == "__main__":
    asyncio.run(auto_delete_incomplete_users())
