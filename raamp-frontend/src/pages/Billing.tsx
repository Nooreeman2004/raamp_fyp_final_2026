        ...
        <motion.div variants={fadeInUp}>
          <div className="grid md:grid-cols-2 gap-6">
              ...
              {tier !== "free" && (
                <div className="mt-6">
                  <motion.div variants={hoverScale} initial="rest" whileHover="hover" whileTap="tap">
                    <Button variant="outline" onClick={() => setShowPasswordGate(true)} className="w-full justify-start border-border/50 text-foreground hover:bg-foreground/10 font-mono text-xs h-10">
            ...
            <HolographicCard className="p-6">
              <h2 className="text-xl font-bold mb-1 font-bebas tracking-wide text-foreground">PAYMENT METHOD</h2>
              <p className="text-xs text-muted-foreground mb-6 font-mono">// PRIMARY SOURCE FOR SUBSCRIPTIONS & AD SPEND</p>
              {tier !== "free" ? (
                <div className="p-4 bg-foreground/5 rounded border border-border/50 flex flex-col gap-4 hover:border-primary/30 transition-colors">
                  <div><span className="text-sm font-mono text-foreground block mb-1">Connected via Stripe</span><span className="text-xs text-muted-foreground font-mono">Your payment methods are securely stored in the Stripe portal. Update your card, download invoices, and manage billing there.</span></div>
                  <Button variant="default" onClick={() => setShowPasswordGate(true)} className="w-full bg-foreground/10 hover:bg-foreground/15 text-foreground border border-border/50 font-mono shadow-none">
                    <CreditCard className="w-4 h-4 mr-2" /> MANAGE PAYMENT METHODS
                  </Button>
                </div>
              ) : (
                <div className="p-4 bg-foreground/5 rounded border border-border/50 flex items-center justify-between hover:border-primary/30 transition-colors">
